  ...
  struct Upgrade_Info fts_updateinfo[] =
{
	...
	{0x64,"FT6336GU",TPD_MAX_POINTS_2,AUTO_CLB_NONEED,10, 10, 0x79, 0x1c, 10, 2000},
...
//		if((get_suspend_state() == PM_SUSPEND_MEM) && (state ==1))
...
#endif
			//fts_read_reg(i2c_client, 0x01, &state); 
			//printk("[FTS] fts_read_reg state_1111 = 0x%x\n",state1);
		...
		if (tpd_proximity_flag == 1)
		{
			...
			fts_read_reg(i2c_client, 0x01, &state); 
			printk("[FTS] fts_read_reg state_2222= 0x%x\n",state);
...
//wangcq327 add end
	...
	#else
		mutex_lock(&i2c_access);
	    fts_write_reg(i2c_client, 0Xa5, 0X03);  //TP enter sleep mode
		//i2c_smbus_write_i2c_block_data(i2c_client, 0xA5, 3, &data);  
		mutex_unlock(&i2c_access);
		msleep(10);
	#endif
