...
static unsigned char CTPM_FW1[] = {
	#include "HQ_ZAL732_Ofilm_FT6336U_V11_20160517_app.i"
};
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
#ifdef FTS_Upgrade_Protect
	if (Vender_ID== TP_VENDOR_ID2)
	{
		pbt_buf = CTPM_FW2;
		fw_len = sizeof(CTPM_FW2);
		printk("xuefeng1 fw_len %d",fw_len);
	}
	else if  (Vender_ID==TP_VENDOR_ID1) 
	{
		pbt_buf = CTPM_FW1;
		fw_len = sizeof(CTPM_FW1);
		printk("xuefeng2 fw_len %d",fw_len);
	}
	else
	{
	...
	if ((fts_updateinfo_curr.CHIP_ID==0x36))
	{
		...
		if (i_ret != 0)
			printk("xuefeng3 fw_len %d",fw_len);
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x64))
	{
		if (fw_len < 8 || fw_len > 54 * 1024) //48*1024
		{
			printk("xuefeng4 fw_len %d",fw_len);
			dev_err(&client->dev, "%s:FW length error\n", __func__);
			return -EIO;
		}
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x54))
	{
		...
		{
			printk("xuefeng5 fw_len %d",fw_len);
			dev_err(&client->dev, "%s:FW length error\n", __func__);
			return -EIO;
		}
		...
		if (i_ret != 0)
			printk("xuefeng6 fw_len %d",fw_len);
		dev_err(&client->dev, "%s:upgrade failed. err.\n",
				__func__);
		return i_ret;
	}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	int i_ret;
	 ...
	 printk("[FTS] uc_tp_vendor_id = 0x%x, uc_host_vendor_id = 0x%x\n",uc_tp_vendor_id, uc_host_vendor_id);
       ...
       if (Vender_ID== TP_VENDOR_ID1)
        {
		uc_host_fm_ver = fts_ofg_ctpm_get_i_file_ver();
        }
       else if (Vender_ID==TP_VENDOR_ID2) 
       {
...
int fts_6336GU_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	printk("[fts]%s:upgrade start.\n",__func__);

	printk("linmingchuang pbt_buf[0] = 0x%x,dw_lenth = 0x%x\n",
		pbt_buf[0], dw_lenth);
	...
	if(dw_lenth > 0x11f)
	{
		...
		if(dw_lenth < fw_length)
		{
			printk("[FTS] Fw length is invalid_11111 \n");
			return -1;
		}
	}
	else
	{
		printk("[FTS] Fw length is invalid_2222 \n");
		return -1;
	}
	...
	{
		/*********Step 1:Reset  CTPM *****/
		fts_write_reg(client, FTS_RST_CMD_REG2, FT_UPGRADE_AA);
		msleep(fts_updateinfo_curr.delay_aa);
		fts_write_reg(client, FTS_RST_CMD_REG2, FT_UPGRADE_55);
		msleep(fts_updateinfo_curr.delay_55);
