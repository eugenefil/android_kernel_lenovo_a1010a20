...
#if 0
if(g_I2cClient->addr != 0x38)
	{
		g_I2cClient->addr = 0x38;
		printk("linmingchuang:i2c_client_FT->addr=%d\n",g_I2cClient->addr);
	}
#endif
...
{
    printk("linmingchuang8 TPD init device driver\n");

#ifdef CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
    tpd_get_dts_info();
#else
    printk("linmingchuang8s TPD init device driver\n");
    i2c_register_board_info(I2C_BUS_ID, &i2c_tpd, 1);
#endif //CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
    if (tpd_driver_add(&tpd_device_driver) < 0)
    {
        printk("linmingchuang8 TPD add MStar TP driver failed\n");
    }
