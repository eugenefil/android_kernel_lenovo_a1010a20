...
//#include "../../../misc/meditek/mach/mt6580/include/mach/mt_pm_ldo.h"
#include <pmic_drv.h>
...
static int _DrvPlatformLyrFingerTouchHandler(void *pUnUsed)
{
    ...
    //DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__); 
    printk("\r\n chenjun=>_DrvPlatformLyrFingerTouchHandler");
...
void DrvPlatformLyrTouchDeviceRegulatorPowerOn(bool nFlag)
{
    ...
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
     ...
     hwPowerOn(PMIC_APP_CAP_TOUCH_VDD, VOL_2800, "TP"); // For specific MTK BB chip(ex. MT6582), need to enable this function call for correctly power on Touch IC.
...
#endif
    //DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__); 
    printk("\r\n chenjun=>DrvPlatformLyrTouchDevicePowerOn");
...
//    gpio_set_value(MS_TS_MSG_IC_GPIO_RST, 1); 
    ...
    mdelay(25); 
#else
    mt_set_gpio_mode(2, GPIO_CTP_RST_PIN_M_GPIO);
    mt_set_gpio_dir(2, GPIO_DIR_OUT);
    mt_set_gpio_out(2, GPIO_OUT_ONE);  
    udelay(100); 

    mt_set_gpio_mode(2, GPIO_CTP_RST_PIN_M_GPIO);
    mt_set_gpio_dir(2, GPIO_DIR_OUT);
    mt_set_gpio_out(2, GPIO_OUT_ZERO);  
    udelay(100); 
...
s32 DrvPlatformLyrInputDeviceInitialize(struct i2c_client *pClient)
{
    ...
    //DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__); 
	printk("\r\n chenjun=>DrvPlatformLyrInputDeviceInitialize");
...
s32 DrvPlatformLyrTouchDeviceRequestGPIO(struct i2c_client *pClient)
{
    ...
    nRetVal = gpio_request(2, "C_TP_RST");     
    if (nRetVal < 0)
    {
...
s32 DrvPlatformLyrTouchDeviceRegisterFingerTouchInterruptHandler(void)
{
    ...
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__); 
	printk("\r\n chenjun=>DrvPlatformLyrTouchDeviceRegisterFingerTouchInterruptHandler");
