...
static void switch_micbias(unsigned long b)
{
		...
		i_current_status = ((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0)>>6); //A=bit1; B=bit0
	     ACCDET_DEBUG("linmingchuang1 = %d\n", i_current_status);


		if (i_current_status == 0)
        {
        	if(swap_4732 == 0){
            ACCDET_DEBUG("linmingchuang2 = %d\n", i_current_status);
		            accdet_FSA8049_enable();
		            accdet_switch = 1;					
        		}else if(swap_4732 == 1){
            ACCDET_DEBUG("linmingchuang3 = %d\n", i_current_status);
   ...
   int irq_temp = 0;
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		ACCDET_DEBUG("ACCDET use in 2.8V mode!! \n");
		    ...
		    //ACCDET_DEBUG("linmingchuang4 [Accdet] FSA8049 enable!\n");
...
//Disable ACCDET
		...
		mutex_unlock(&accdet_eint_irq_sync_mutex);
		  ...
		  cable_pin_recognition = 0;
		    ...
		    pmic_pwrap_write(0x0400, pmic_pwrap_read(0x0400)&~(1<<14)); 
		    //accdet_FSA8049_disable();  //disable GPIO209 for PIN swap 
		    ACCDET_DEBUG("linmingchuang5 [Accdet] FSA8049 disable!\n");
		#endif
...
static ssize_t proc_set_accdet_current(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
#if 0
