...
static imgsensor_struct imgsensor = {
	...
	.autoflicker_en = KAL_FALSE1,  //auto flicker enable: KAL_FALSE for disable auto flicker, KAL_TRUE for enable auto flicker
	.test_pattern = KAL_FALSE1,		//test pattern mode or not. KAL_FALSE for in test pattern mode, KAL_TRUE for normal output
...
static void set_max_framerate(UINT16 framerate,kal_bool_sensor min_framelength_en)
{
...
static kal_uint32 open(void)
{
	...
	imgsensor.autoflicker_en= KAL_FALSE1;
	...
	imgsensor.dummy_line = 0;
	imgsensor.ihdr_en = 0;
	imgsensor.test_pattern = KAL_FALSE1;
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	imgsensor.pclk = imgsensor_info.pre.pclk;
	//imgsensor.video_mode = KAL_FALSE1;
	imgsensor.line_length = imgsensor_info.pre.linelength;
	imgsensor.frame_length = imgsensor_info.pre.framelength; 
	imgsensor.min_frame_length = imgsensor_info.pre.framelength;
	imgsensor.autoflicker_en = KAL_FALSE1;
...
static kal_uint32 set_video_mode(UINT16 framerate)
{
	...
	if (framerate == 0)
		...
		return ERROR_NONE;
	spin_lock(&imgsensor_drv_lock);
	if ((framerate == 300) && (imgsensor.autoflicker_en == KAL_TRUE1))
		imgsensor.current_fps = 296;
	else if ((framerate == 150) && (imgsensor.autoflicker_en == KAL_TRUE1))
...
static kal_uint32 set_auto_flicker_mode(kal_bool_sensor enable, UINT16 framerate)
{
	//LOG_INF("enable = %d, framerate = %d \n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
	if (enable) //enable auto flicker	  
		imgsensor.autoflicker_en = KAL_TRUE1;
...
static kal_uint32 set_test_pattern_mode(kal_bool_sensor enable)
{
	//LOG_INF("enable: %d\n", enable);
