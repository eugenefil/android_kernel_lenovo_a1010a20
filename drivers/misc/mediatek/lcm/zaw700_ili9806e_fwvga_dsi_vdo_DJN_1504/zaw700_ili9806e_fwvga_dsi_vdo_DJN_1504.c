...
{	
	...
	params->dsi.PLL_CLOCK=200;
			//params->dsi.PLL_CLOCK=214; //add by xuefeng for sheping test
...
IMM_GetOneChannelValue(1,&data,&lcm_adc); //read lcd _id
...
#if defined(BUILD_LK)
			  printf("ili9806e_DJN_dsi lk -- ili9806e 0x%x , 0x%x , 0x%x, 0x%x,%d\n", id_high, id_midd, id_low, id,lcm_adc);
#else
			  printk("ili9806e_DJN_dsi kernel -- ili9806e 0x%x , 0x%x , 0x%x, 0x%x,%d \n", id_high, id_midd, id_low, id,lcm_adc);
#endif
	...
	if((0x0604 == chip_id) && (lcm_adc > 2500))
		return 1;
	else
		return 0;
...
printk("xuefeng===DJ Kernel ");//add by xuefeng
	return TRUE;
}
