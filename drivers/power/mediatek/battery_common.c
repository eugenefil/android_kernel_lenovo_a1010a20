...
extern kal_int32 gFG_capacity_by_v; //add by liunianliang for runin
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if (BMT_status.UI_SOC <= 1) {
		set_rtc_spare_fg_value(1);
                  ...
                  // modify by liunianliang for runin 2016.04.23 begin
...
extern kal_uint32 get_charging_setting_current();
...
int set_lenovo_charger_status()
{
	...
	else if(temperature > 10 && temperature <= 45)//650
	{
...
//add by yinyapeng 20160317 ==>
	...
	printk("linmingchuang10  add g_call_state = %d\n",g_call_state);
	printk("linmingchuang11 g_call_state = %d\n",g_lcmSuspend);

	if (upmu_is_chr_det() == KAL_TRUE) {
	printk("linmingchuang12  add g_call_state = %d\n",g_call_state);
	printk("linmingchuang13 g_call_state = %d\n",g_lcmSuspend);
		if (g_call_state == 1 && g_lcmSuspend == 1) {
				 set_bat_charging_current_limit(400);
		}	
	}
...
//liuxiong add 20131122 start
			...
			case BAT_THREAD_CTRL: /* for runtime test */
                             if (*user_data_addr == 4){
                                   battery_log(BAT_LOG_CRTI, "[BATTERY] 1 runing set_rtc_spare_fg_value =%d\n",gFG_capacity_by_v);
                                  ...
                                  battery_log(BAT_LOG_CRTI, "[BATTERY] 2 runing set_rtc_spare_fg_value =%d\n",gFG_capacity_by_v);
...
static ssize_t proc_set_charge_current(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
...
static ssize_t current_cmd_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d", &cmd_current_unlimited, &cmd_discharging) == 2) {
		...
		if (cmd_discharging == 1) {
			charging_enable = false;
                             g_runin_stop_thermal = KAL_TRUE; //add by liunianliang for runin
			adjust_power = -1;
		} else if (cmd_discharging == 0) {
			charging_enable = true;
                             g_runin_stop_thermal = KAL_FALSE; //add by liunianliang for runin
			adjust_power = -1;
		}
